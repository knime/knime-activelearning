...
public class LabelModelAdapterTest {
    ...
    private static float[][] COVARIANCE_MATRIX;
    ...
    public static void initClass() throws FileNotFoundException, IOException {
        ...
        COVARIANCE_MATRIX = covMat(AUGMENTED_LABEL_MATRIX);
    ...
    public void testInitialize() throws Exception {
        ...
        m_testInstance.initialize(COVARIANCE_MATRIX, mask, CLASS_BALANCE, PREC_INIT, lr);
    ...
    public void testGetProbabilities() throws Exception {
        ...
        assertMatrixEquals(conditionalProbabilities, m_testInstance.calculateProbabilities(AUGMENTED_LABEL_MATRIX),
            1e-5f);
    }

    private void train() {
        m_testInstance.initialize(COVARIANCE_MATRIX, MASK, CLASS_BALANCE, PREC_INIT, 0.01f);
        for (int i = 0; i < 100; i++) {
    ...
    private static float[][] covMat(final float[][] matrix) {
        final float[][] covMat = new float[matrix[0].length][matrix[0].length];
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[0].length; j++) {
                for (int k = 0; k < matrix[0].length; k++) {
                    covMat[j][k] += matrix[i][j] * matrix[i][k];
                }
            }
        }
        for (int i = 0; i < covMat.length; i++) {
            for (int j = 0; j < covMat.length; j++) {
                covMat[i][j] /= matrix.length;
            }
        }
        return covMat;
    }
