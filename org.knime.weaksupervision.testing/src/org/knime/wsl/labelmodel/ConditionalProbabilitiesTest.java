...
public class ConditionalProbabilitiesTest {
    ...
    @Test
    public void testGetConditionalProbability() throws Exception {
        final float[][] mu =
            fm(
                fa(1.1112f, 0.2326f),
                fa(0.0285f, 0.4610f),
        ...
        final float[][] expected =
            fm(
                ...
                fa(0.5912f, 0.0391f),
                fa(0.3585f, 0.4093f));
        final ConditionalProbabilities cProbs = new ConditionalProbabilities(mu);
        for (int lf = 0; lf < 2; lf++) {
            for (int y = 1; y < 3; y++) {
                for (int val = 0; val < 3; val++) {
                    assertEquals(expected[lf * 3 + val][y - 1], cProbs.getConditionalProbability(lf, val, y), 1e-3f);
                }
            }
        }
    }
